...
> Trạng thái: **Chờ duyệt PLAN** (danh sách này sinh sẵn từ PLAN.md để duyệt một lượt; chỉ thực thi sau khi
...
- [ ] **T1.1** Xóa ghi đè `role: 'ADMIN'` trong `verifyJWT` (`src/lib/auth.ts`).
      *DoD:* decode token của staff trả `role: 'STAFF'`. Ước tính: 0.5h.
- [ ] **T1.2** Trả đúng role thật trong response của `login`, `me`, `register` route.
      *DoD:* `GET /api/auth/me` với token staff trả `"role": "STAFF"`. Ước tính: 0.5h.
- [ ] **T1.3** Đăng nhập UI bằng cả 4 vai trò seed, soát các trang dashboard không crash/trắng trang khi
      `role !== 'ADMIN'`.
...
- [ ] **T2.1** Thêm model `DocumentCounter` vào `prisma/schema.prisma`, tạo migration kèm SQL backfill
...
- [ ] **T2.2** Viết lại `generateDocumentCode(tx, prefix)` — chữ ký mới bắt buộc nhận transaction client;
...
- [ ] **T2.3** Cập nhật 9 route sinh mã sang chữ ký mới, chuyển điểm gọi vào trong `$transaction`
      ...
      *DoD:* `npx tsc --noEmit` sạch; tạo thử 1 bản ghi mỗi loại qua API đều thành công, mã nối tiếp đúng.
      Ước tính: 3h.
- [ ] **T2.4** Riêng `quotations/[id]/convert`: gộp 2 lần sinh mã còn 1, sinh QR sau khi có mã thật trong
      transaction.
      *DoD:* convert 1 báo giá → hóa đơn có mã và QR chứa đúng mã đó, bộ đếm chỉ tăng 1. Ước tính: 1h.

## Nhóm 3 — FR-3: Chặn bán vượt kho

- [ ] **T3.1** Thêm kiểm tra tồn kho + trừ kho bằng `decrement` có điều kiện trong `invoices/route.ts`,
      ...
      *DoD:* tạo hóa đơn vượt kho bị chặn, tồn kho và StockMovement không đổi; hóa đơn hợp lệ trừ kho đúng.
      Ước tính: 2h.
- [ ] **T3.2** *(Chờ trả lời câu hỏi 1 trong PLAN)* Áp cùng logic trừ kho + kiểm tồn cho
...
- [ ] **T4.1** Viết `scripts/verify-gd1.mjs` chạy 7 tiêu chí chấp nhận của SPEC bằng API thật, in bảng
      PASS/FAIL.
      *DoD:* chạy `node scripts/verify-gd1.mjs` trên app đang chạy → 7/7 PASS. Ước tính: 2h.
- [ ] **T4.2** Chạy verify trên bản Docker prod (`--profile prod`), backup DB trước khi test.
      *DoD:* 7/7 PASS trên container prod; file backup mới trong `backups/`. Ước tính: 0.5h.
- [ ] **T4.3** Cập nhật tài liệu: tích các mục GĐ1 trong `DANH-GIA-VA-LO-TRINH.md`, thêm bài học mới (nếu
