...
export default function NewQuotationPage() {
  ...
  // Cập nhật giá trị số lượng, giá, chiết khấu, VAT trong dòng
  const handleItemValueChange = (index: number, field: keyof QuotationItemInput, val: string) => {
    ...
    // Tính toán lại thành tiền của dòng
  ...
  // Tính toán tóm tắt tổng số tiền báo giá
  const getTotals = () => {
    let subtotal = 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Kiểm tra các dòng chưa chọn sản phẩm
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tight">Lập Báo giá Mới</h1>
          <p className="text-sm text-muted-foreground">Nhập thông tin khách hàng, lựa chọn sản phẩm và tỷ lệ chiết khấu.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3 bg-card border border-border p-5 rounded-2xl shadow-sm text-xs">
          ...
          {/* Ngày hạn thanh toán / báo giá */}
          <div>
            <label className="block font-bold mb-1.5 text-muted-foreground uppercase tracking-wider">Hạn báo giá (Tùy chọn)</label>
            <input
              type="date"
              value={dueDate}
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Báo giá có hiệu lực trong 15 ngày..."
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          <div className="w-full md:max-w-md text-xs text-muted-foreground bg-muted/40 p-4 rounded-xl border border-border space-y-1">
            <p className="font-bold text-foreground">Lưu ý khi tính giá trị chứng từ:</p>
            <p>1. Tổng tiền hàng = Số lượng × Đơn giá bán.</p>
            <p>2. Giá sau chiết khấu = Tổng tiền hàng − (Tổng tiền hàng × Chiết khấu % / 100).</p>
            <p>3. Thuế VAT = Giá sau chiết khấu × Thuế VAT % / 100.</p>
            <p>4. Tổng cộng thanh toán = Giá sau chiết khấu + Thuế VAT.</p>
          </div>
          ...
          <div className="w-full md:w-80 bg-card border border-border p-5 rounded-2xl shadow-sm space-y-3.5 text-xs">
            ...
            >
              {submitting ? 'Đang tạo báo giá...' : 'Lập & Lưu báo giá'}
            </button>
          </div>
        </div>
